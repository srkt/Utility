...
(SHEET_NAME, EXCEL_COL_POSITION, EXCEL_ROW1_HEADER, EXCEL_ROW2_HEADER, EXCEL_ROW3_HEADER,
 ...
 STG_TABLE_NAME, STG_COLUMN_NAME, FACT_TABLE_NAME, FACT_COLUMN_NAME,
 DATA_TYPE, IS_DIMENSION, IS_REQUIRED, IS_STOP_COLUMN, IS_ACTIVE)
SELECT 'Savings Forecast',
       r.EXCEL_COL_POSITION + CASE WHEN r.EXCEL_COL_POSITION >= 8 THEN 1 ELSE 0 END,
       r.EXCEL_ROW1_HEADER, r.EXCEL_ROW2_HEADER, r.EXCEL_ROW3_HEADER,
       r.ROW1_EXPECTED, r.ROW2_EXPECTED, r.FUZZY_THRESHOLD,
       'CES_SAVINGS_STG_FORECAST',
       CASE WHEN r.STG_COLUMN_NAME = 'REPORTING_PERIOD' THEN 'FORECAST_PERIOD' ELSE r.STG_COLUMN_NAME END,
       'CES_SAVINGS_FACT_FORECAST',
       CASE WHEN r.FACT_COLUMN_NAME = 'REPORTING_PERIOD' THEN 'FORECAST_PERIOD' ELSE r.FACT_COLUMN_NAME END,
       r.DATA_TYPE, r.IS_DIMENSION,
       CASE WHEN r.STG_COLUMN_NAME = 'REPORTING_PERIOD' THEN 'Y' ELSE r.IS_REQUIRED END,
       r.IS_STOP_COLUMN, r.IS_ACTIVE
FROM CES_SAVINGS_COLUMN_MAP r
WHERE r.SHEET_NAME = 'Savings Results';
